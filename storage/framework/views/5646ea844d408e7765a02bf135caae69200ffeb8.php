            ...
            <table id="example" class="table mt-2 mb-2 table-striped table-bordered"
                ...
                <tbody class="text-center">
                    <?php
                        ...
                        $aturanMinum = json_decode($item->obat->obat_Ro_aturan, true) ?? [];
                        $anjuranMinum = json_decode($item->obat->obat_Ro_anjuran, true) ?? [];
                        $jumlahObat = json_decode($item->obat->obat_Ro_jumlah, true) ?? [];
                        $jenisObat = json_decode($item->obat->obat_Ro_jenisObat, true) ?? [];

                        // Fallback ke SOAP jika kosong
                        if (empty($semuaObat) && $soap) {
                            $semuaObat = json_decode($soap->soap_p ?? '[]', true) ?? [];
                        }
                        if (empty($aturanMinum) && $soap) {
                            $aturanMinum = json_decode($soap->soap_p_aturan ?? '[]', true) ?? [];
                        }
                        if (empty($anjuranMinum) && $soap) {
                            $anjuranMinum = json_decode($soap->soap_p_anjuran ?? '[]', true) ?? [];
                        }
                        if (empty($jumlahObat) && $soap) {
                            $jumlahObat = json_decode($soap->soap_p_jumlah ?? '[]', true) ?? [];
                        }
                        if (empty($jenisObat) && $soap) {
                            $jenisObat = json_decode($soap->soap_p_jenis ?? '[]', true) ?? [];
                        }
