...
namespace App\Http\Controllers\Perawat\Laporan\PoliUmum;
...
class UmumUmumController extends Controller
{
    public function poliUmumPasienUmum(Request $request)
    {
        ...
        $umumUmum = $query->paginate($entries, ['*'], 'page', $page);
        $umumUmum->appends([
            'search' => $search,
            'entries' => $entries,
        ...
        $umumUmum->getCollection()->transform(function ($item) {
            $kdDiagnosa = [];
            $nmDiagnosa = [];
        ...
        Log::info('Paginasi Data BPJS', [
            'total' => $umumUmum->total(),
            'per_page' => $umumUmum->perPage(),
            'current_page' => $umumUmum->currentPage(),
            'search' => $search,
            'filter_option' => $filterOption,
            'tanggal' => $tanggal,
            'month' => $month,
        ...
        return view('perawat.laporan.poliUmum.umum.pasienUmum', compact('umumUmum', 'search', 'entries', 'filterOption', 'tanggal', 'month', 'tahun', 'months'));
    }

    public function exportExcelUmumUmum(Request $request)
    {
        $search = $request->input('search');
        $filterOption = $request->input('filter_option');
        $tanggal = $request->input('tanggal');
        ...
        return Excel::download(
            new RekapPasienUmumExport($search, $filterOption, $tanggal, $month, $tahun),
            'pasien_umum_poli_umum_' . now()->format('Ymd_His') . '.xlsx'
        );
    }
}
