...
class GigiBpjsController extends Controller
{
    public function poliGigiPasienBpjs(Request $request)
    {
        ...
        $gigiBpjs = $query->paginate($entries, ['*'], 'page', $page);
        $gigiBpjs->appends([
            'search' => $search,
            'entries' => $entries,
        ...
        $gigiBpjs->getCollection()->transform(function ($item) {
            $kdDiagnosa = [];
            $nmDiagnosa = [];
        ...
        Log::info('Paginasi Data BPJS', [
            'total' => $gigiBpjs->total(),
            'per_page' => $gigiBpjs->perPage(),
            'current_page' => $gigiBpjs->currentPage(),
            'search' => $search,
            'filter_option' => $filterOption,
            'tanggal' => $tanggal,
            'month' => $month,
        ...
        return view('perawat.laporan.poliGigi.bpjs.pasienBpjs', compact('gigiBpjs', 'search', 'entries', 'filterOption', 'tanggal', 'month', 'tahun', 'months'));
    }

    public function exportExcelPoliGigiBpjs(Request $request)
    {
        $search = $request->input('search');
        $filterOption = $request->input('filter_option');
        $tanggal = $request->input('tanggal');
        ...
        return Excel::download(
            new RekapPasienGigiBpjsExport($search, $filterOption, $tanggal, $month, $tahun),
            'pasien_bpjs_poli_gigi_' . now()->format('Ymd_His') . '.xlsx'
        );
    }
}
